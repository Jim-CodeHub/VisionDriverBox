...
*/
 /**
\~english 
 * @brief Get the accessmode of opened device
 * @param[in] hDev Camera device handle
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_HANDLE
 * @see ItkDevOpen
 */
IKAPC_API ITKSTATUS IKAPC_CC ItkDevGetCameraAccessMode(ITKDEVICE hDev, uint32_t* pAccessmode);
...
/**
\~chinese 
 * @brief 相机固件升级,函数成功返回后需要等待相机状态灯变为绿色时，断上电以使升级生效。本函数不检查版本兼容性，需谨慎使用
 * @param[in] hDev 相机设备句柄
 * @param[in] strFileName 本地固件文件
 * @return
 * - ITKSTATUS_OK
...
*/
 /**
\~english 
 * @brief Upgrade device's firmware. When the function returns successfully, the camera should be repowered when status light 
    turns green. This function does not check version compatibility, please use with caution.
 * @param[in] hDev Camera device handle
 * @param[in] strFileName local firmware file
 * @return
 * - ITKSTATUS_OK
 ...
 * @see ItkDevGetUpgradeProcess
 */
IKAPC_API ITKSTATUS IKAPC_CC ItkDevUpgradeWithNoLimit(ITKDEVICE hDev, const char* strFileName);
 ...
 *  @param[in]  hDev  相机的句柄
 *  @param[in]  index  数据流索引，有效范围[0, ItkDevGetStreamCount -1]。如果为ITKSTREAM_VAL_BUFFER_NUM_SELF_ADAPTION，则数量由内部自适应确定。
 ...
 *  @param[in]  index Data stream index, valid range is [0, ItkDevGetStreamCount -1]. If is ITKSTREAM_VAL_BUFFER_NUM_SELF_ADAPTION, the number of buffers is determined internally.
 ...
 * | ITKDEV_PRM_OVERLAP_MODE                         |
 * | ITKDEV_PRM_OVERLAP_HEIGHT                       |
 * @see ItkDevSetPrm
*/
 /**
\~english 
 ...
 * @brief 设置以太网相机传输模式
 * @param[in] hDev 相机设备句柄
 * @param[in] pstTransmissionType 传输模式结构体
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_NOT_IMPLEMENT
 * @see ITK_GIGE_TRANSMISSION_TYPE
 * @note 若要使用组播，应该在ItkDevOpen之后，ItkStreamStart之前调用；若不设置组播，可以不调用
*/
 /**
\~english 
 * @brief Set Gige camera transmission type 
 * @param[in] hDev Camera device handle
 * @param[in] pstTransmissionType TransmissionType struct
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_INVALID_HANDLE
 * - ITKSTATUS_NOT_IMPLEMENT
 * @see ITK_GIGE_TRANSMISSION_TYPE
 * @note should call this after ItkDevOpen and before ItkStreamStart if multicast is need, or should not call this
 */
IKAPC_API ITKSTATUS IKAPC_CC ItkGigEDevSetTransmissionType(
    ITKDEVICE hDev, ITK_GIGE_TRANSMISSION_TYPE* pstTransmissionType);

/**
\~chinese 
 * @brief 获取相机的组播状态
 * @param[in] pDevInfo 获取组播状态相机的基本信息
 * @param[out] pMulticastStatus 组播状态
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_NOT_IMPLEMENT
 * @see ITK_GIGE_MULTICAST_STATUS
*/
 /**
\~english 
 * @brief Get Gige camera multicast status
 * @param[in] pDevInfo the base infomation used to specify camera which to get multicast status
 * @param[out] pMulticastStatus Multicast status
 * @return
 * - ITKSTATUS_OK
 * - ITKSTATUS_INVALID_ARG
 * - ITKSTATUS_NOT_IMPLEMENT
 * @see ITK_GIGE_MULTICAST_STATUS
 */
IKAPC_API ITKSTATUS IKAPC_CC ItkGigEDevGetMulticastStatus(ITKDEV_INFO* pDevInfo, ITK_GIGE_MULTICAST_STATUS* pMulticastStatus);
 ...
 * 8. BILINEAR, VNG, DFSD/DFCD/DFCD2 和 ITEK_OLD, 从左到右插值效果逐渐变好，处理速度逐渐变慢。
 * 其中Dfcd是Dfsd的优化版本，在大多数场景下相比Dfsd插值效果更好、速度更快；Dfcd2相较于Dfcd，对类似黑白棋盘格的场景角点的颜色恢复效果更好。
 ...
 * 8. For BILINEAR, VNG, DFSD/DFCD/DFCD2 and ITEK_OLD, the interpolation quality improves progressively from left to right, while processing speed gradually decreases. 
 * DFCD, an optimized version of DFSD, delivers superior interpolation performance and faster processing speed in most scenarios. 
 * DFCD2 further enhances color restoration at corner points in checkerboard-like patterns compared to DFCD.
 * @see ITKBUFFER_VAL_BAYER_PLANNAR
 */
 ...
 * | [0:7]   | 图像保存格式                                       |
 * | [8:23]  | 仅用于设置Jpeg图像质量系数，范围1~100               |
 * | [24]    | 仅用于pcd和ply数据，0表示ASCII，1表示二进制         |
 * | [25]    | 仅用于ply数据，0表示不保存曲面数据，1表示保存曲面数据 |
 * | [26:31] | 预留                                              |
 * @see ItkBufferLoad
*/
 /**
 ...
 * | ITKBUFFER_VAL_PCD                  |
 * | ITKBUFFER_VAL_PLY                  |
 * @note The description of saving configuration options is as follows:
 * | Bit     | Description                                                                 |
 * |---------|-----------------------------------------------------------------------------|
 * | [0:7]   | Image save format                                                           |
 * | [8:23]  | Only used to set Jpeg image quality coefficient, ranging from 1 to 100      |
 * | [24]    | Only used for pcd and ply images, 0 represents ASCII, 1 represents binary   |
 * | [25]    | Only used for ply images, 0: Do not save surface data, 1: Save surface data |
 * | [26:31] | Reserved                                                                    |
 * @see ItkBufferLoad
 */
