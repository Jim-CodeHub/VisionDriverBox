...
#define IKStatus_CommandMissRequirement   0x00000014 ///< @brief \~chinese 串口指令不符合要求 \~english The serial port command does not meet the requirements
#define IKStatus_CommandNotAllow          0x00000015 ///< @brief \~chinese 当前不允许串口指令 \~english The serial port command is not allowed
...
#define IKStatus_CommandResultNoExit      0x00000019 ///< @brief \~chinese 相机执行结果无意义 \~english Camera execution result is meaningless
...
#define IKStatus_Set_Info_Error           0x00000027 ///< @brief \~chinese 设置参数值无效或者错误 \~english Invalid or wrong setting parameter value
#define IKStatus_Grab_Pending             0x00000028 ///< @brief \~chinese 当前有正在执行的图像采集过程 \~english Image acquisition process is in progress
#define IKStatus_Insufficient_Resource    0x00000029 ///< @brief \~chinese 系统没有足够的资源 \~english System does not have enough resources
#define IKStatus_Grab_Abort               0x0000002a ///< @brief \~chinese 放弃当前采集 \~english Abandon the current acquisition
#define IKStatus_Need_Reboot              0x0000002b ///< @brief \~chinese 系统出现致命错误，必须重启计算机以恢复 \~english A fatal system error occurred which must restart computer to solve
#define IKStatus_Need_Restart             0x0000002c ///< @brief \~chinese 采集卡出现错误，需重新打开采集卡以恢复 \~english An frame grabber error occurred which must reopen the frame grabber to solve
#define IKStatus_Not_Implement            0x0000002d ///< @brief \~chinese 功能未实现 \~english Function not implemented
#define IKStatus_Not_In_Callback          0x0000002e ///< @brief \~chinese 该功能应在回调函数中运行，但实际不在 \~english This function should be run in callback function, but actually not
#define IKStatus_Not_Match_Driver_Version 0x0000002f ///< @brief \~chinese 驱动版本不匹配 \~english Driver version mismatch
#define IKStatus_Prm_Not_Available        0x00000030 ///< @brief \~chinese 参数当前不可读不可写 \~english The parameter is currently unreadable and not writable
#define IKStatus_Prm_Locked               0x00000031 ///< @brief \~chinese 采集卡参数锁定，无法修改 \~english The parameters of frame grabber are locked and cannot be modified
 ...
 * 2. 当前帧为不定长采图最后一帧且图像完整时，此时此帧的 ( uVaryFrameEnd &0xffff)  等于0，下一帧的( uVaryFrameEnd >>16)表示此次不定长采图的缓冲区个数。
 */
 /**
\~english
...
( uVaryFrameEnd &0xffff) of this frame is equal to 0, and the ( uVaryFrameEnd >>16) of the next frame represents the number of buffers for
...
\~chinese
 * @brief 缓冲区part数量
 * @note 用于表示当前帧中的part数量.
 */
 /**
\~english
 * @brief Buffer parts num
 * @note Indicates the the buffer parts num of this buffer. 
 */
    unsigned long long uPartNum;
    unsigned Reserved[46];  ///< @brief \~chinese 预留字段 \~english Reserved field
 ...
 * @see IKapGetBufferPartInfo
 */
typedef struct _ikap_buffer_part_info {
    uint8_t         HasChunkData;           ///< @brief \~chinese 是否有chunkdata \~english Whether chunk data exists
    uint8_t         Reserved8[3];
    int32_t         PartDataType;           ///< @brief \~chinese 表示缓冲区part的数据类型, 参考 GenTL::PARTDATATYPE_IDs \~english Indicates the type of buffer part, refer to GenTL::PARTDATATYPE_IDs
    void*           Address;                ///< @brief \~chinese 表示缓冲区part的首地址 \~english Indicates the start address of the buffer part
    uint64_t        Width;                  ///< @brief \~chinese 缓冲区part宽度 \~english Width of the buffer part
    uint64_t        Height;                 ///< @brief \~chinese 缓冲区part高度 \~english Height of the buffer part
    uint64_t        PixelFormat;            ///< @brief \~chinese 缓冲区part像素格式 \~english Pixel format of the buffer part
    uint64_t        ValidSize;              ///< @brief \~chinese 缓冲区part有效尺寸 \~english Valid size of the buffer part
    uint64_t        TotalSize;              ///< @brief \~chinese 缓冲区part总尺寸 \~english Total size of the buffer part
    ...
    uint64_t        ValidHeight;            ///< @brief \~chinese 缓冲区part有效高度 \~english Valid height of the buffer part
    uint64_t        Reserved[9];            ///< @brief \~chinese 预留 \~english Reserved
    char            Name[32];               ///< @brief \~chinese 缓冲区part名字 \~english Name of the buffer part name
    char            PixelFormat_Str[64];    ///< @brief \~chinese 缓冲区part像素格式 \~english Pixel format of the buffer part
} IKAPBUFFERPARTINFO, *PIKAPBUFFERPARTINFO;
 ...
 * 2. 当resourceName为空时，函数返回IKStatus_InvalidParameter，但resourceNameSize会输出采集卡名称的有效长度。
...
IKapGetBoardCount .
 * 2. When resourceName is empty, the function will return IKStatus_InvalidParameter，but resourceNameSize will output valid length of board name.
 ...
 *  @see IKapGetBoardCount
 */
IKAPBOARDDLL int IKAPBOARD_CC IKapGetBoardName(unsigned resourceType, unsigned resourceIndex, char* resourceName, unsigned* resourceNameSize);
 ...
 *  @param[in] resourceType 采集卡资源类型
 *  @param[in] resourceIndex 采集卡索引
 *  @param[out] pProductModel 采集卡产品型号
 *  @param[in,out] productModelSize productModel缓冲区长度
 * @return
 * - IK_RTN_OK
 * - IK_RTN_ERROR
 * - IKStatus_BoardNotFound
 ...
 *  @note
 * 1. 采集卡索引的有效范围为[0,n-1]，其中n是通过 IKapGetBoardCount 获得的resourceCount值。
 * 2. 当productModel为空时，函数返回 IKStatus_InvalidParameter, 但是productModeleSize会输出采集卡产品型号的有效长度。
 * 3. productModeleSize作为输入时，指明的是输入缓冲区productModel的最大长度；作为输出时，指明的是实际获取的产品型号的有效长度。
 当输入缓冲区过小时，函数调用将会失败，此时会返回 IKStatus_BufferTooSmall 错误代码并在productModelSize中存放采集卡的缓冲区大小。
 * 4. 采集卡资源类型resourceType可选项如下所示：
 * | 采集卡资源类型 |
 * |----------------|
 * | IKBoardALL     |
...
/**
\~english
 *  @brief Get the product model of frame grabber
 *  @param[in] resourceType Type of frame grabbers
 *  @param[in] resourceIndex Frame grabber index
 *  @param[out] pProductModel Product model of frame grabber
 *  @param[in,out] productModelSize productModel buffer length
 * @return
 * - IK_RTN_OK
 * - IK_RTN_ERROR
 * - IKStatus_BoardNotFound
 ...
 * 1. The valid range for frame grabber index is [0, n-1], where n is the resourceCount value obtained through
IKapGetBoardCount .
 * 2. When productModel is a null ptr, the function will return IKStatus_InvalidParameter, but the productModelSize will output the valid length of product model of frame grabber.
 * 3. When productModelSize is used as input, it indicates the maximum length of input buffer productModel. When
used as output, it indicates effective length of name actually get. When input buffer is too small, function will fail,
returning IKStatus_BufferTooSmall error code and storing buffer size of the frame grabber product model in productModelSize.
 * 4. Type of frame grabbers resourceType options are as follows:
 * | Type of frame grabbers |
 * |------------------------|
 * | IKBoardALL             |
 ...
 *  @see IKapGetBoardCount
 */
IKAPBOARDDLL int IKAPBOARD_CC IKapGetProductModel(unsigned resourceType, unsigned resourceIndex, char* productModel, unsigned* productModelSize);
...
该接口仅使用于CL采集卡。 该函数主要适用于不使用IKapC打开相机的场景, 如果使用IKapC打开相机, 请使用 IKapOpenWithSpecificInfo .

 * 1. 采集卡资源类型resourceType可选项如下所示：
 * | 采集卡资源类型resourceType |
 * |----------------------------|
 * | IKBoardALL                 |
 * @see IKapClose IKapOpenWithSpecificInfo
*/
/**
\~english
...
Only applicable to CL frame grabbers. This function is mainly applicable to scenarios where the camera is not opened with IKapC. 
If the camera is opened with IKapC, please use IKapOpenWithSpecificInfo instead.
 ...
 * 4. 输入路径在windows下使用本地多字符集编码(不能使用非本地多字符集编码的文字), 在linux下使用utf-8编码.
 ...
 * 4. The input path uses the local multi-byte character set (MBCS) encoding on Windows, and UTF-8 encoding on Linux.
 * @see IKapSaveConfigurationToFile
 */
 ...
 * - IKStatus_ConfigFilePathInvalid
 * - IKStatus_Invalid_Handle
 * @note 本函数必须指明配置文件的全部路径。 输入路径在windows下使用本地多字符集编码(不能使用非本地多字符集编码的文字), 在linux下使用utf-8编码.
 ...
 *  The input path uses the local multi-byte character set (MBCS) encoding on Windows, and UTF-8 encoding on Linux.
 * @see IKapLoadConfigurationFromFile
 */
 ...
 * @brief 获取缓冲区Part信息
 * @param[in] hDev 采集卡句柄
 * @param[in] nFrameIndex 缓冲区索引，有效范围[0~ IKP_FRAME_COUNT -1]
 * @param[in] nPartIndex 缓冲区part索引, 有效范围[0~ partNum -1]. partNum可以通过IKapGetBufferStatusEx获取.
 * @param[out] pIKapBufferPartInfo 缓冲区part信息
 * @return
 * - IK_RTN_OK
 * - IK_RTN_ERROR
 * - IKStatus_BoardNotOpen
...
*/
 /**
\~english
 * @brief Get buffer part information
 * @param[in] hDev Acquisition card handle
 * @param[in] nFrameIndex Buffer index, valid range [0 ~ IKP_FRAME_COUNT - 1]
 * @param[in] nPartIndex Buffer part index, valid range [0 ~ partNum - 1]. partNum can be obtained via IKapGetBufferStatusEx.
 * @param[out] pIKapBufferPartInfo Buffer part information
 * @return
 * - IK_RTN_OK
 * - IK_RTN_ERROR
 * - IKStatus_BoardNotOpen
 ...
 * @see IKapGetBufferStatusEx
 */
IKAPBOARDDLL int IKAPBOARD_CC IKapGetBufferPartInfo(HANDLE hDev, int nFrameIndex, int64_t nPartIndex, PIKAPBUFFERPARTINFO pIKapBufferPartInfo);
/**
\~chinese
 * @brief 释放缓冲区
 * @param[in] hDev 采集卡句柄
 ...
 * @brief 读取采集卡端DDR数据
 * @param[in] hDev 采集卡句柄
 * @param[in] nAddress 读取起始地址
 * @param[in] nReadLength 读取长度
 * @param[out] pBuffer 数据接收缓冲区
 * @return
 * - IK_RTN_OK
 * - IKStatus_Invalid_Handle
 * - IKStatus_Not_Implement
 * @note 该功能仅固件版本为2.9.0及以上的CXP12-4采集卡支持。
 * @see IKapWriteDDR
*/
 /**
\~english
 * @brief Read DDR data from board
 * @param[in] hDev Frame grabber handle
 * @param[in] nAddress The start address for reading
 * @param[in] nReadLength The read length
 * @param[out] pBuffer The buffer for receiving data
 * @return
 * - IK_RTN_OK
 * - IKStatus_Invalid_Handle
 * - IKStatus_Not_Implement
 ...
 * @see IKapWriteDDR
 */
IKAPBOARDDLL int IKapReadDDR(HANDLE hDev, uint32_t nAddress, uint64_t nReadLength, void* pBuffer);

/**
\~chinese
 * @brief 写入DDR数据到采集卡端
 * @param[in] hDev 采集卡句柄
 * @param[in] nAddress 写入起始地址
 * @param[in] nReadLength 数据长度
 * @param[out] pBuffer 写入数据
 * @return
 * - IK_RTN_OK
 * - IKStatus_Invalid_Handle
 * - IKStatus_Not_Implement
...
*/
 /**
\~english
 * @brief Write DDR data to board
 * @param[in] hDev Frame grabber handle
 * @param[in] nAddress The start address for writing
 * @param[in] nReadLength The write length
 * @param[out] pBuffer The buffer for writing data
 * @return
 * - IK_RTN_OK
 * - IKStatus_Invalid_Handle
 * - IKStatus_Not_Implement
 ...
 * @see IKapReadDDR
 */
IKAPBOARDDLL int IKapWriteDDR(HANDLE hDev, uint32_t nAddress, uint64_t nWriteLength, void* pBuffer);
