...
#    define IKAPC_API __attribute__((visibility("default")))
#    if defined(__i386__)
#        define IKAPC_CC __attribute__((stdcall))
#    else
#        define IKAPC_CC
#    endif
#    define IKAPC_PACKING 8
...
/**
\~chinese
 * @brief 相机 Overlap 模式【RW】
 * @note 本参数用于设置相机的 Overlap（重叠拼接）模式，单位为“模式枚举值”。仅 GigEVision（GV）相机生效。
 * 1. 该模式用于连续采集等场景中，相邻两帧图像之间保留一段重叠区域，便于拼接或算法对齐。
 * 2. 本参数对应的 pValue/value 为 uint32_t 类型数据。
 * 3. 参数取值及描述如下所示：
 * | 参数取值 | 描述                                                         |
 * |----------|--------------------------------------------------------------|
 * | 0        | 关闭 Overlap                                                 |
 * | 1        | 前向重叠：当前帧的上端部分来自前一帧（Top from previous）     |
 * | 2        | 后向重叠：当前帧的下端部分来自后一帧（Bottom from next）      |
 * @see ITKDEV_PRM_OVERLAP_HEIGHT ITKDEV_VAL_OVERLAP_MODE_OFF ITKDEV_VAL_OVERLAP_MODE_TOP_FROM_PREV ITKDEV_VAL_OVERLAP_MODE_BOTTOM_FROM_NEXT
*/
 /**
\~english
 * @brief Camera Overlap mode [RW]
 * @note This parameter configures the camera Overlap mode. It is effective only for GigEVision (GV) cameras.
 * 1. Overlap is used to keep an overlapped region between adjacent frames in continuous acquisition,
 *    which is helpful for stitching or algorithm alignment.
 * 2. The corresponding pValue/value of this parameter is uint32_t type data.
 * 3. The parameter values and descriptions are as follows:
 * | Parameter value | Description                                                     |
 * |-----------------|-----------------------------------------------------------------|
 * | 0               | Disable Overlap                                                 |
 * | 1               | Forward overlap: the top part of the current frame comes from the previous frame (Top from previous) |
 * | 2               | Backward overlap: the bottom part of the current frame comes from the next frame (Bottom from next) |
 * @see ITKDEV_PRM_OVERLAP_HEIGHT ITKDEV_VAL_OVERLAP_MODE_OFF ITKDEV_VAL_OVERLAP_MODE_TOP_FROM_PREV ITKDEV_VAL_OVERLAP_MODE_BOTTOM_FROM_NEXT
 */
#define ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM(5, 4)

/**
\~chinese
 * @brief 相机 Overlap 高度【RW】
 * @note 本参数用于设置 Overlap 重叠区域的高度（重叠行数/像素行数）。仅 GigEVision（GV）相机生效。
 * 1. 该高度表示相邻两帧之间被重复采集/复用的区域大小；其含义需与 @ref ITKDEV_PRM_OVERLAP_MODE 的模式配合理解。
 * 2. 本参数对应的 pValue/value 为 uint32_t 类型数据。
 * 3. 取值范围：0 ~ 图像高度（含边界）。
 * @see ITKDEV_PRM_OVERLAP_MODE
*/
 /**
\~english
 * @brief Camera Overlap height [RW]
 * @note This parameter sets the height of the overlapped region (number of overlapped lines/pixel rows).
 * It is effective only for GigEVision (GV) cameras.
 * 1. The height indicates the size of the region that is duplicated/reused between adjacent frames; interpret it together
 *    with the mode configured by @ref ITKDEV_PRM_OVERLAP_MODE.
 * 2. The corresponding pValue/value of this parameter is uint32_t type data.
 * 3. Valid range: 0 to image height (inclusive).
 * @see ITKDEV_PRM_OVERLAP_MODE
 */
#define ITKDEV_PRM_OVERLAP_HEIGHT ITKDEV_PRM(6, 4)
...
\~chinese
 * @brief Overlap 模式：关闭
 * @note 关闭相机 Overlap（重叠）功能，不产生帧间重叠区域。
 * @see ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM_OVERLAP_HEIGHT
*/
 /**
\~english
 * @brief Overlap mode: Off
 * @note Disable camera Overlap. No overlapped region will be generated between adjacent frames.
 * @see ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM_OVERLAP_HEIGHT
 */
#define ITKDEV_VAL_OVERLAP_MODE_OFF                 0u

/**
\~chinese
 * @brief Overlap 模式：前向重叠（上端来自前一帧）
 * @note 启用前向重叠后，当前帧图像的上端部分将与前一帧的末端区域重叠（Top from previous）。
 * 重叠区域高度由 @ref ITKDEV_PRM_OVERLAP_HEIGHT 指定。仅 GigEVision（GV）相机生效。
 * @see ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM_OVERLAP_HEIGHT
*/
 /**
\~english
 * @brief Overlap mode: Forward overlap (top from previous)
 * @note When enabled, the top part of the current frame overlaps with the tail region of the previous frame
 * (Top from previous). The overlap height is configured by @ref ITKDEV_PRM_OVERLAP_HEIGHT.
 * Effective only for GigEVision (GV) cameras.
 * @see ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM_OVERLAP_HEIGHT
 */
#define ITKDEV_VAL_OVERLAP_MODE_TOP_FROM_PREV       1u

/**
\~chinese
 * @brief Overlap 模式：后向重叠（下端来自后一帧）
 * @note 启用后向重叠后，当前帧图像的下端部分将与后一帧的起始区域重叠（Bottom from next）。
 * 重叠区域高度由 @ref ITKDEV_PRM_OVERLAP_HEIGHT 指定。仅 GigEVision（GV）相机生效。
 * @see ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM_OVERLAP_HEIGHT
*/
 /**
\~english
 * @brief Overlap mode: Backward overlap (bottom from next)
 * @note When enabled, the bottom part of the current frame overlaps with the starting region of the next frame
 * (Bottom from next). The overlap height is configured by @ref ITKDEV_PRM_OVERLAP_HEIGHT.
 * Effective only for GigEVision (GV) cameras.
 * @see ITKDEV_PRM_OVERLAP_MODE ITKDEV_PRM_OVERLAP_HEIGHT
 */
#define ITKDEV_VAL_OVERLAP_MODE_BOTTOM_FROM_NEXT    2u
...
*/
 /**
\~english
 * @brief ply image format
 * @see ItkBufferLoad ItkBufferSave
 */
#define ITKBUFFER_VAL_PLY 6u
...
#define ITKSTREAM_VAL_GRAB_STRATEGY_LAST_ONE 0x00000001u
 ...
 \~chinese
  * @brief 缓冲区数量自适应
  * @note 作为ItkDevAllocStreamEx接口实参使用，以开辟合适的缓冲区数量，最终开辟数量可通过ItkStreamGetBufferNum获知
  * @see ItkDevAllocStreamEx ItkStreamGetBufferNum
 */
 /**
\~english
 * @brief Adaptive buffer allocation
 * @note Used as an argument for the ItkDevAllocStreamEx interface to allocate an appropriate number of buffers. The final number of allocated buffers can be retrieved via ItkStreamGetBufferNum.
 * @see ItkDevAllocStreamEx ItkStreamGetBufferNum
 */
#define ITKSTREAM_VAL_BUFFER_NUM_SELF_ADAPTION 0x00000000u
...
*/
 /**
\~english
 * @brief GigE device transmission type: unicast
 */
#define ITKGIGEDEV_TRANSTYPE_UNICAST 0u
...
*/
 /**
\~english
 * @brief GigE device transmission type: multicast
 */
#define ITKGIGEDEV_TRANSTYPE_MULTICAST 1u

/**
\~chinese
 * @brief 千兆网相机传输类型为组播模式（本实例不接收图像数据）
 * @note 适用于仅参与组播控制但不接收图像数据的场景。
*/
 /**
\~english
 * @brief GigE device transmission type: multicast mode (this instance does not receive image data)
 * @note Applicable for scenarios where the instance only participates in multicast control without receiving image data.
 */
#define ITKGIGEDEV_TRANSTYPE_MULTICAST_WITHOUT_RECV 2u
 ...
 * @note USBVersion 数值含义: 0x0200表示USB 2.0, 0x0110表示USB 1.1, 以此类推.
*/
 /**
\~english 
 * @brief USB30 camera device specific information structure
 * @see ItkManGetU3VDeviceInfo
 * @note USBVersion meaning: A value of 0x0200 indicates USB 2.0, 0x0110 indicates USB 1.1, etc.
 */
typedef struct tag_itk_u3v_dev_info {
    ...
    uint8_t  BusNum;       ///< @brief \~chinese 总线索引 \~english Bus number
    uint8_t  DevAddr;      ///< @brief \~chinese 相机地址 \~english Camera address
    char     Reserved[254]; ///< @brief \~chinese 预留字段 \~english Reserved field
...
\~chinese 
 * @brief 以太网相机传输类型
 * @see ItkGigEDevSetTransmissionType
*/
 /**
\~english 
 * @brief Gige Camera Transmission type
 * @see ItkGigEDevSetTransmissionType
 */
typedef struct _itk_gige_transmision_type
{
    uint32_t            nTransmissionType;              ///< @brief \~chinese 传输类型(0为单播，1为组播，2为组播但本实例不接收图像数据)            \~english Transmission type(0 is unicast，1 is multicast，2 is multicast but this instance does not receive image data)
    uint32_t            nDestIp;                        ///< @brief \~chinese 组播IP（例如239.192.1.1为0xEFC00101） \~english multicast ip address(239.192.1.1 sholud be set as 0xEFC00101)
    uint16_t            nDestPort;                      ///< @brief \~chinese 组播端口                              \~english multicast port
    uint32_t            reversed[8];                    ///< @brief \~chinese 预留                                  \~english reserved
}ITK_GIGE_TRANSMISSION_TYPE, *PITK_GIGE_TRANSMISSION_TYPE;
#   pragma pack (pop)
...
\~chinese 
 * @brief 相机组播状态
 * @see ItkGigEDevGetMulticastStatus
 * @see ItkGigEDevSetTransmissionType
*/
 /**
\~english 
 * @brief Camera Multicast Status
 * @see ItkGigEDevGetMulticastStatus
 * @see ItkGigEDevSetTransmissionType
 */
typedef struct _itk_gige_multicast_status
{
    bool                bIsMulticast;                   ///< @brief \~chinese 是否处于多播状态                      \~english is in multicast status
    uint32_t            nDestIp;                        ///< @brief \~chinese 组播IP（例如239.192.1.1为0xEFC00101） \~english multicast ip address(239.192.1.1 sholud be set as 0xEFC00101)
    uint16_t            nDestPort;                      ///< @brief \~chinese 组播端口                              \~english multicast port
    uint32_t            reversed[8];                    ///< @brief \~chinese 预留                                  \~english reserved
}ITK_GIGE_MULTICAST_STATUS, *PITK_GIGE_MULTICAST_STATUS;
#   pragma pack (pop)
