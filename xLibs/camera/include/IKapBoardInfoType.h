...
/**
\~chinese
 * @brief 通用输入信号1上升沿延迟【RW】
 * @note 本参数表示通用输入信号1上升沿时间延迟。
*/
 /**
\~english
 * @brief General input signal1 rising edge delay [RW]
 * @note This parameter represents general input signal1 rising edge delay.
 */
#define IKP_HARDWARE_TRIGGER_GENERAL_INPUT1_DELAY (0x10000067)

/**
\~chinese
 * @brief 通用输入信号2上升沿延迟【RW】
 * @note 本参数表示通用输入信号2上升沿时间延迟。
*/
 /**
\~english
 * @brief General input signal2 rising edge delay [RW]
 * @note This parameter represents general input signal2 rising edge delay.
 */
...
\~chinese
 * @brief 多帧融合数量【RW】
 * @note 本参数表示多帧融合的数量。如果为1, 则不会进行多帧融合。此参数大于1时，应保证设置缓冲区数量不小于融合数量的两倍
\~english
 * @brief Multi-frame fusion count [RW]
 * @note This parameter represents multi-frame fusion count.When this parameter is greater than 1, 
             ensure that the number of buffers is no less than twice the fusion count.
 */
 ...
 * | 参数值             |  注释                                           |
 * |-------------------|------------------------------------------------|
 * | 0                 |  默认模式, 在TIMEOUT时间内未收到新的数据包即会发生超时 |
 * | 1                 |  在未收到第一个数据包前不会发生超时, 其余情况同默认模式.(仅CL采集卡支持)|                           
*/
 /**
\~english
 * @brief Frame Timeout Mode of the Capture Card【RW】
 * @note This parameter indicates the frame timeout mode of the capture card.
 * | Value | Description |
 * |---|---------------------------------------------------|
 * | 0 | Default mode. A timeout occurs if no new data packet is received within the TIMEOUT period. |
 * | 1 | No timeout occurs until the first data packet is received. Other behaviors are the same as the default mode. |
 */
#define IKP_TIMEOUT_MODE (0x1000009c)

/**
\~chinese
 * @brief 通用输入信号1下降沿延迟【RW】
 * @note 本参数表示通用输入信号1下降沿时间延迟，应大于等于上升沿延迟。
*/
 /**
\~english
 * @brief General input signal1 falling edge delay [RW]
 * @note This parameter represents general input signal1 falling edge delay, should not less then rising edge delay.
 * When it is less than the rising edge delay, the actual value takes the value of the rising edge delay.
 */
#define IKP_HARDWARE_TRIGGER_GENERAL_INPUT1_DELAY_FALLINGEDGE (0x1000009d)

/**
\~chinese
 * @brief 通用输入信号2下降沿延迟【RW】
 * @note 本参数表示通用输入信号2下降沿时间延迟。
*/
 /**
\~english
 * @brief General input signal2 falling edge delay [RW]
 * @note This parameter represents general input signal2 falling edge delay, should not less then rising edge delay.
 * When it is less than the rising edge delay, the actual value takes the value of the rising edge delay.
 */
#define IKP_HARDWARE_TRIGGER_GENERAL_INPUT2_DELAY_FALLINGEDGE (0x1000009e)

/**
\~chinese
 * @brief CC1通道行触发信号控制功能使能【RW】
 * @note 本参数表示是否开启CC1通道的控制功能，开启后卡端在帧触发上升沿使能输出，在收集满定长行数后禁止输出。
*/
 /**
\~english
 * @brief Enable the line trigger signal control function of CC1 Channel [RW]
 * @note This parameter indicates whether to enable the control function of the CC1 channel. After enabling, 
 * the card side enables the output on the frame trigger rising edge and disables the output after collecting the 
 * full specified number of lines.
 */
#define IKP_CC1_CHANNEL_CONTROL_ENABLE (0x1000009f)

/**
\~chinese
 * @brief 加载出厂配置文件
 * @note 本参数设置后会加载采集卡出厂配置文件。【WO】。
*/
 /**
\~english
 * @brief Load the factory configuration file
 * @note This parameter setting will load the factory configuration file of the acquisition card. [WO].
 */
#define IKP_LOAD_FACTORY_CONFIGURATION   (0x100000A0)
...
\~chinese
 * @brief 定点比较器0FIFO【WO】
 * @note 本参数表示积分信号1数量达到阈值时，触发一次定点比较器0 FIFO输出。如果设置值小于积分信号数量，设置后会立即触发一次FIFO输出。64bit。支持CL K6卡
 * 执行开始采集与IKP_EVENT_SIG_COUNT_RESET后阈值会清空，之后软件端确保每次写入阈值需大于上次写入值，直到下次信号清空或开始采集后重新开始保护。
*/
 /**
\~english
 * @brief Position comparator0 FIFO [WO]
 * @note This parameter represents that one FIFO0 output is triggered when the quantity of integrated signal 1 reaches the threshold.
 * If the set value is less than the integrated signal count, a FIFO output will be triggered immediately after the setting is applied.
 * After initiating acquisition and executing IKP_POSITION_COMPARATOR_RESET, the threshold is cleared. The software ensures that each 
 * subsequently written threshold value is greater than the last. Signal clearance or the start of a new acquisition will reset the protection.
 */
#define IKP_POSITION_COMPARATOR0_FIFO (0x20000025)

/**
\~chinese
 * @brief 定点比较器0FIFO【WO】
 * @note 本参数表示积分信号1数量达到阈值时，触发一次定点比较器1 FIFO输出。如果设置值小于积分信号数量，设置后会立即触发一次FIFO输出。64bit。支持CL K6卡
 * 执行开始采集与IKP_EVENT_SIG_COUNT_RESET后阈值会清空，之后软件端确保每次写入阈值需大于上次写入值，直到下次信号清空或开始采集后重新开始保护。
*/
 /**
\~english
 * @brief Position comparator1 FIFO [WO]
 * @note This parameter represents that one FIFO1 output is triggered when the quantity of integrated signal 1 reaches the threshold.
 * If the set value is less than the integrated signal count, a FIFO output will be triggered immediately after the setting is applied.
 * After initiating acquisition and executing IKP_POSITION_COMPARATOR_RESET, the threshold is cleared. The software ensures that each 
 * subsequently written threshold value is greater than the last. Signal clearance or the start of a new acquisition will reset the protection.
 */
#define IKP_POSITION_COMPARATOR1_FIFO (0x20000026)

/**
\~chinese
 * @brief 定点比较器0输出脉冲宽度(us)【RW】
 * @note 本参数表示定点比较器0输出的脉冲宽度。支持CL K6卡
*/
 /**
\~english
 * @brief Position comparator0 output pulse width(us) [RW]
 * @note This parameter represents the pulse width of position comparator0.
 */
#define IKP_POSITION_COMPARATOR0_OUTPUT_WIDTH (0x20000027)

/**
\~chinese
 * @brief 定点比较器1输出脉冲宽度(us)【RW】
 * @note 本参数表示FIFO0输出的脉冲宽度。支持CL K6卡
*/
 /**
\~english
 * @brief Position comparator1 output pulse width(us) [RW]
 * @note This parameter represents the pulse width of position comparator1.
 */
#define IKP_POSITION_COMPARATOR1_OUTPUT_WIDTH (0x20000028)

/**
\~chinese
 * @brief 定点比较器0输出脉冲延迟时间(us)【RW】
 * @note 本参数表示定点比较器0的输出脉冲延迟。支持CL K6卡
*/
 /**
\~english
 * @brief Position comparator0 pulse delay(us) [RW]
 * @note This parameter represents the pulse delay of position comparator0.
 */
#define IKP_POSITION_COMPARATOR0_OUTPUT_DELAY (0x20000029)

/**
\~chinese
 * @brief 定点比较器1输出脉冲延迟时间(us)【RW】
 * @note 本参数表示定点比较器1的输出脉冲延迟。支持CL K6卡
*/
 /**
\~english
 * @brief Position comparator1 pulse delay(us) [RW]
 * @note This parameter represents the pulse delay of position comparator1.
 */
#define IKP_POSITION_COMPARATOR1_OUTPUT_DELAY (0x20000030)

/**
\~chinese
 * @brief 定点比较器重置【WO】
 * @note 本参数表示重置定点比较器位置设置，同时清空积分信号、编码器信号。支持CL K6卡
*/
 /**
\~english
 * @brief Position comparator reset [WO]
 * @note This parameter represents reseting the position comparator settings and the counts for the integration signal, encoder signal.
 */
#define IKP_POSITION_COMPARATOR_RESET (0x20000031)

/**
\~chinese
 * @brief 积分信号1计数锁存模式1【RW】
 * @note 本参数表示积分信号1计数的锁存模式1。锁存模式下只有检测到对应信号，才会更新积分信号计数。支持CL K6卡
 * 1. 积分信号1计数锁存模式可选项如下所示：
 * | 积分信号1计数锁存模式                       |
 * |---------------------------------------------|
 * | IKP_INTEGRATION_SIG1_NON_LATCH              |
 * | IKP_INTEGRATION_SIG1_GENERAL_INPUT1_LATCH   |
 * | IKP_INTEGRATION_SIG1_GENERAL_INPUT2_LATCH   |
*/
 /**
\~english
 * @brief Integration signal 1 counting latch mode 1 [RW]
 * @note This parameter represents integration signal 1 counting mode. In latch mode, the integral signal count is updated 
 * only when the corresponding signal is detected.
 * 1. Integration signal 1 count latch mode options are as follows:
 * | Integration signal 1 count latch mode       |
 * |---------------------------------------------|
 * | IKP_INTEGRATION_SIG1_NON_LATCH              |
 * | IKP_INTEGRATION_SIG1_GENERAL_INPUT1_LATCH   |
 * | IKP_INTEGRATION_SIG1_GENERAL_INPUT2_LATCH   |
 */
#define IKP_INPUT_INTEGRATION_SIG_1_COUNTING_LATCH_MODE_1 (0x20000032)

/**
\~chinese
 * @brief 积分信号1计数锁存模式2【RW】
 * @note 本参数表示积分信号1计数的锁存模式2。锁存模式下只有检测到对应信号，才会更新积分信号计数。支持CL K6卡
 * 1. 积分信号1计数锁存模式可选项如下所示：
 * | 积分信号1计数锁存模式                       |
 * |---------------------------------------------|
 * | IKP_INTEGRATION_SIG1_NON_LATCH              |
...
*/
 /**
\~english
 * @brief Integration signal 1 counting latch mode 2 [RW]
 * @note This parameter represents integration signal 1 counting mode. In latch mode, the integral signal count is updated 
 * only when the corresponding signal is detected.
 * 1. Integration signal 1 count latch mode options are as follows:
 * | Integration signal 1 count latch mode       |
 ...
 * | IKP_INTEGRATION_SIG1_GENERAL_INPUT2_LATCH   |
 */
#define IKP_INPUT_INTEGRATION_SIG_1_COUNTING_LATCH_MODE_2 (0x20000033)

/**
\~chinese
 * @brief 积分信号1计数锁存1【RO】
 * @note 本参数表示从采集卡读取的锁存模式1下的积分信号1个数。同IKP_EVENT_INPUT_INTEGRATION_SIG_1_COUNT
*/
 /**
\~english
 * @brief Integration signal1 counter latch1 [RO]
 * @note This parameter represents the number of integration signal1 under latch mode1 read from frame grabber. 
 * Same as IKP_EVENT_INPUT_INTEGRATION_SIG_1_COUNT
 */
#define IKP_EVENT_INPUT_INTEGRATION_SIG_1_COUNT_LATCH1 (0x20000034)

/**
\~chinese
 * @brief 积分信号1计数锁存2【RO】
 * @note 本参数表示从采集卡读取的锁存模式2下的积分信号1个数。
*/
 /**
\~english
 * @brief Integration signal1 counter latch2 [RO]
 * @note This parameter represents the number of integration signal1 under latch mode2 read from frame grabber.
 */
#define IKP_EVENT_INPUT_INTEGRATION_SIG_1_COUNT_LATCH2 (0x20000035)
...
/**
\~english
 * @brief CXP/CL image header tag recovery mode [RW]
...
\~chinese
 * @brief 编码器平滑窗口长度【RW】
 * @note 本参数表示参与编码器平滑窗口的编码器样本数量。 
 * 编码器平滑窗口功能会统计一段时间内的编码器触发周期平均值，并按此平均值产生新的编码器触发信号。当前仅支持SFP-4采集卡.
*/
 /**
\~english
 * @brief Shaft encoder smoothing windows size [RW]
 * @note This parameter indicates the number of encoder samples involved in the encoder smoothing window.
 * The encoder smoothing window function calculates the average value of encoder trigger cycles over a period of time and 
 * generates new encoder trigger signals based on this average value.
 */
#define IKP_SHAFT_ENCODER_SMOOTHING_WINDOW_SIZE (0x30000046)

/**
\~chinese
 * @brief 编码器平滑窗口超时时间【RW】
 * @note 本参数表示编码器平滑窗口的超时时间, 超过此间隔时间没有有效编码器输入（经过分倍频和正反转补偿后的有效编码器触发）时会自动复位。当前仅支持SFP-4采集卡.
*/
 /**
\~english
 * @brief Shaft encoder smoothing windows timeout gap [RW]
 * @note This parameter indicates the timeout period of the encoder smoothing window.
 * When no valid encoder input (valid encoder triggers after frequency division, multiplication, 
 * and forward/reverse rotation compensation) is received beyond this interval, 
 * the system will be automatically reset.
 */
#define IKP_SHAFT_ENCODER_SMOOTHING_WINDOW_TIMEOUT_GAP (0x30000047)

/**
\~chinese
 * @brief 图像Y拉伸【RW】
 * @note 本参数表示是否使能图像Y拉伸模式, 开始采图后每两行间以插值的方式插入1行，行数、行频均会加倍。仅适用于8b模式。
*/
 /**
\~english
 * @brief Image Y Stretch [RW]
 * @note This parameter indicates whether to enable the Image Y Stretch mode. 
 After image acquisition starts, one line is inserted between every two lines by interpolation, with both the line count and line frequency doubled. 
 Applicable only to 8b mode.
 */
#define IKP_IMAGE_Y_STRETCH (0x30000048)
...
/**
\~chinese
 * @brief 标识LOGIC_PLUG功能是否开启【RW】
 * @note 该功能仅固件版本为2.9.0及以上的CXP12-4采集卡支持。
*/
 /**
\~english
 * @brief Indicates whether the LOGIC_PLUG function is enabled [RW]
 * @note This function is only supported by CXP12-4 frame grabber with firmware version 2.9.0 or later.
 */
#define IKP_LOGIC_PLUG_ENABLE (0x3100000B)

/**
\~chinese
 * @brief LOGIC_PLUG功能寄存器偏移量【RW】
 * @note 该功能仅固件版本为2.9.0及以上的CXP12-4采集卡支持。
*/
 /**
\~english
 * @brief The address of LOGIC_PLUG register [RW]
 * @note This function is only supported by CXP12-4 frame grabber with firmware version 2.9.0 or later.
 */
#define IKP_LOGIC_PLUG_REGISTER_OFFSET (0x3100000C)   

/**
\~chinese
 * @brief LOGIC_PLUG功能寄存器值【RW】
 * @note 该功能仅固件版本为2.9.0及以上的CXP12-4采集卡支持。
*/
 /**
\~english
 * @brief The value of LOGIC_PLUG register [RW]
 * @note This function is only supported by CXP12-4 frame grabber with firmware version 2.9.0 or later.
 */
#define IKP_LOGIC_PLUG_REGISTER_VALUE (0x3100000D)    

/**
\~chinese
 * @brief LOGIC_PLUG功能开放的DDR内存空间【RO】
 * @note 该功能仅固件版本为2.9.0及以上的CXP12-4采集卡支持。
*/
 /**
\~english
 * @brief The size of DDR memory available for LOGIC_PLUG function [RO]
 * @note This function is only supported by CXP12-4 frame grabber with firmware version 2.9.0 or later.
 */
#define IKP_LOGIC_PLUG_MAX_DDR_SIZE (0x3100000E)   
...
#define IKP_IMAGE_TYPE_VAL_RGBC        (2)	///< @brief \~chinese 彩色RGBC图像，其中C是图像的亮度信息 \~english RGBC, where C is grayscale information of image
...
#define IKP_BOARD_TRIGGER_MODE_VAL_CXP_LEVEL_VALID          (2)	///< @brief \~chinese 电平有效模式 \~english CXP frame grabber level effective mode
#define IKP_BOARD_TRIGGER_MODE_VAL_FRAME_ACTIVE             (3)	///< @brief \~chinese 高电平帧有效模式 \~english CXP frame grabber frame active mode
#define IKP_BOARD_TRIGGER_MODE_VAL_FRAME_ACTIVE_ALL_LEVELS  (4)	///< @brief \~chinese 全部电平帧有效模式 \~english CXP frame grabber frame active all levels mode
///@}
...
#define IKP_GENERAL_OUTPUT_SOURCE_VAL_WATCHDOG              (10)	///< @brief \~chinese 看门狗信号 \~english Watch trigger signal
#define IKP_GENERAL_OUTPUT_SOURCE_VAL_POSITION_COMPARATOR0  (11)	///< @brief \~chinese FIFO0信号 \~english FIFO0 signal
#define IKP_GENERAL_OUTPUT_SOURCE_VAL_POSITION_COMPARATOR1  (12)	///< @brief \~chinese FIFO1信号 \~english FIFO1 signal
#define IKP_GENERAL_OUTPUT_SOURCE_VAL_All_Low               (13)	///< @brief \~chinese 持续低电平信号 \~english All Low signal
#define IKP_GENERAL_OUTPUT_SOURCE_VAL_All_High              (14)	///< @brief \~chinese 持续高电平信号 \~english All High signal
///@}
...
\~chinese
 * @name 积分信号1计数锁存模式
 * @see IKP_INPUT_INTEGRATION_SIG_1_COUNTING_LATCH_MODE_1 IKP_INPUT_INTEGRATION_SIG_1_COUNTING_LATCH_MODE_2
*/
 /**
\~english
 * @name Integration signal 1 count latch mode options
 * @see IKP_INPUT_INTEGRATION_SIG_1_COUNTING_LATCH_MODE_1 IKP_INPUT_INTEGRATION_SIG_1_COUNTING_LATCH_MODE_2
 */
#define IKP_INTEGRATION_SIG1_NON_LATCH (0)	///< @brief \~chinese 非锁存状态，实时变化 \~english Non-latched state, changing in real-time
#define IKP_INTEGRATION_SIG1_GENERAL_INPUT1_LATCH   (1)	///< @brief \~chinese General input 1 锁存 \~english General input 1 latch
#define IKP_INTEGRATION_SIG1_GENERAL_INPUT2_LATCH   (2)	///< @brief \~chinese General input 2 锁存 \~english General input 1 latch
///@}
...
#define IKP_CXP_FIRMWARE_TYPE_1_CAMERA_NEW (4)  ///< @brief \~chinese 1拖1 \~english 1-1
